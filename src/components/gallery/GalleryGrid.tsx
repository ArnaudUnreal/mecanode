...
type Filter = { kind: 'all' } | { kind: 'category' | 'tag'; slug: string }

const isActive = (filter: Filter, kind: string, slug?: string) =>
  filter.kind === kind && (filter.kind === 'all' || filter.slug === slug)

/**
 * Galerie filtrable. Le filtrage se fait dans le navigateur : la page reste statique
 * et le visiteur ne subit aucun aller-retour serveur.
 */
    ...
    <button
      ...
      className={`font-mono text-[11px] tracking-[0.07em] uppercase border px-3.5 py-[7px] transition-colors ${
        active
          ...
          : 'border-line bg-panel text-ink-2 hover:border-[#2E4356] hover:text-ink'
      }`}
      key={key}
      onClick={onClick}
      type="button"
    ...
    <div>
      ...
      {visible.length === 0 ? (
        <div className="clip-node-lg border border-line bg-panel px-6 py-14 text-center">
          ...
          <button
            className="clip-node border border-line bg-transparent px-[18px] py-2.5 text-[13.5px] font-semibold transition-colors hover:border-[#3A2F55] hover:bg-panel-2"
